...
for (start = 0; start < total_length; start += irand(5,15))
{
        ...
        MBANDEDWG(start, irand(8,12), irand(1500,2000)*bowlenv, cpspch(pitches[thispitch]), 0.3, 1, 0.5, 3, 0.0, 1.0, 0.0, random())
        thispitch = trand(pitchlen-1)
        MBANDEDWG(start, irand(8,12), irand(1500,2000)*bowlenv, cpspch(pitches[thispitch]), 0.3, 1, 0.5, 3, 0.0, 1.0, 0.0, random())
...
for (start = 5; start < total_length; start += tempo[which_tempo])
{
        ...
        pan = irand(0.4,0.6)
        MMESH2D(start+humanize_tempo, 1, 8000*start/total_length, trand(2,7), trand(2,7), 0.8, 0.9, 1.0, irand(0.8,1), pan)
        if (random()<0.1)
