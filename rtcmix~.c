...
void *rtcmix_new(long num_inoutputs, long num_additional)
{
	...
	x->symbol = NSLookupSymbolInModule(x->module, "_rtcmixmain");
	if (x->symbol == NULL)
		error("cannot find rtcmixmain, looked here: %s", pathname);
	else {
		x->rtcmixmain = NSAddressOfSymbol(x->symbol);
			if (x->rtcmixmain)	x->rtcmixmain();
			else error("rtcmix~ could not call rtcmixmain()");
	}

	x->symbol = NSLookupSymbolInModule(x->module, "_maxmsp_rtsetparams");
	if (x->symbol == NULL)
		error("cannot find maxmsp_rtsetparams, looked here: %s", pathname);
	else {
		x->maxmsp_rtsetparams = NSAddressOfSymbol(x->symbol);
	...
	x->symbol = NSLookupSymbolInModule(x->module, "_pullTraverse");
	if (x->symbol == NULL)
		error("cannot find pullTraverse, looked here: %s", pathname);
	else {
		x->pullTraverse = NSAddressOfSymbol(x->symbol);
		if (!(x->pullTraverse))
	...
	if (x->symbol == NULL)
		error("cannot find parse_dispatch, looked here: %s", pathname);
	else {
	...
	x->symbol = NSLookupSymbolInModule(x->module, "_check_error");
	if (x->symbol == NULL)
		error("cannot find check_error, looked here: %s", pathname);
	else {
		x->check_error = NSAddressOfSymbol(x->symbol);
		if (!(x->check_error))
	...
	if (x->symbol == NULL)
		error("cannot find pfield_set, looked here: %s", pathname);
	else {
	...
	x->symbol = NSLookupSymbolInModule(x->module, "_buffer_set");
	if (x->symbol == NULL)
		error("cannot find buffer_set, looked here: %s", pathname);
	else {
	...
	x->symbol = NSLookupSymbolInModule(x->module, "_flush_sched");
	if (x->symbol == NULL)
		error("cannot find flush_sched, looked here: %s", pathname);
	else {
...
void rtcmix_dsp(t_rtcmix *x, t_signal **sp, short *count)
{
	...
	for(i = 0; i < (x->num_inputs + x->num_pinlets + x->num_outputs); i++) { //pointers to the input and output vectors
