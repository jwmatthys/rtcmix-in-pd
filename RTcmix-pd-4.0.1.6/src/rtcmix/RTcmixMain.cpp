...
// BGG mm -- for the [flush] message (flush_sched(), resetQueueHeap())
#include "heap.h"
   ...
   */
   if (rtInteractive) {
      ...
      if (!noParse) {
         ...
         cout << "Parsing once ...\n";
#endif
// BGG mm -- we should never be here in max/msp (i.e. noParse == 1)
...
// BGG mm -- set status to 0 so that the rest will go ok for max-msp
//  the parse() function is called from max-msxp now
...
// BGG mm -- called by flush_sched() in main.cpp (for the [flush] message)
void
