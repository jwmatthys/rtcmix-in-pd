...
//-----------------------------------------------------------------------------
// BGG mm -- take input from a buffer, not a file; "fd" is the buffer pointer
//		looks like *str_src can do this, but not implemented.  I'm lazy.
//t_CKBOOL Chuck_Compiler::go( const string & filename, FILE * fd, const char * str_src )
t_CKBOOL Chuck_Compiler::go( const string & filename, char *fd, const char * str_src )
{
    t_CKBOOL ret = TRUE;
...
t_CKBOOL Chuck_Compiler::do_normal( const string & filename, char *fd, const char * str_src )
{
    t_CKBOOL ret = TRUE;
    Chuck_Context * context = NULL;
...
// BGG mm
    EM_log( CK_LOG_SYSTEM, "module maxmsp..." );
    load_module( env, maxmsp_query, "maxmsp", "global" );
