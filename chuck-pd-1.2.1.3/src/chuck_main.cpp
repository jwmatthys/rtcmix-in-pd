...
//#endif
// no command-line args in max/msp -- pass in the relevant params directly
int chuckmain(int buffer_size, int srate)
{
...
// BGG mm -- see above, we're passing these in from max/msp as args
...
// BGG mm -- we don't do these in max/msp
/*
    // check buffer size
...
// BGG mm -- added the type casts
    if( !vm->initialize( enable_audio, vm_halt, (t_CKUINT)srate, (t_CKUINT)buffer_size,
...
// BGG mm -- no signalling in max/msp
/*
    // catch SIGINT
...
// BGG mm -- the compiler runs from  the parse_compile() function below
// input comes from buffers passed from max/msp
/*
...
// BGG mm -- don't do this in max/msp
/*
    // boost priority
...
// BGG mm -- the following functions are for the max/msp interface

void pull_cb2(float *outbuf, int bufsize)
{
   Digitalio::cb2((char *)outbuf, bufsize, g_vm);
}

int
parse_score(char *thebuf)
{
   Chuck_VM_Code * code = NULL;
   Chuck_VM_Shred * shred = NULL;

   // parse, type-check, and emit
   if( !g_compiler->go("max-msp", thebuf) )
      return 1;

   // get the code
   code = g_compiler->output();
   // name it
   code->name += string("max-msp");

   // spork it
   shred = g_vm->spork( code, NULL );

   // BGG mm -- I should probably check the return of "shred" for errors
   return 0;
}


// BGG -- this will be called from max/msp, the ChucK ugen maxbang will set
// the bang_ready value as a signal to return 1 instead of 0
...
// BGG -- this function is called once every vector from max/msp.
// the ChucK maxmessage ugen will fill the maxmsp_vals[] array
// each vector (possible with more than 1 value if maxmessage is
// repeatedly called), and the vals_ready will be incremented
// max/msp will then bundle this up and send out a list

int vals_ready = 0;
float maxmsp_vals[1000]; // 1000 is arbitrary, probably should malloc
...
// this is set from inlets on the chuck~ object to send values into
// a variable dynamically
float inletvals[20];

int inlet_set(int inlet, float val, int numinlets)
{
   if (inlet > numinlets) {
      fprintf(stderr, "You asked for a value from inlet #%d, but you have only declared %d inlets on the [chuck~] object.\n", inlet, numinlets);
      return(0);
   }
   inletvals[inlet-1] = val;
   return(1);
}
