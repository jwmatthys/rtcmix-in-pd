...
// desc: ...
//-----------------------------------------------------------------------------
// BGG mm -- take input from a buffer, not a file
//t_CKBOOL chuck_parse( c_constr fname, FILE * fd, c_constr code )
t_CKBOOL chuck_parse( c_constr fname, char *buf, c_constr code )
{
	t_CKBOOL clo = FALSE;
    t_CKBOOL ret = FALSE;

// BGG mm -- using "buf" to pass in the max/msp chuck code
/*
    // sanity check
    if( fd && code )
    {
...
// BGG mm -- this was commented out in the chuck source.  Probably the better
//    way to do it
    /*
...
// BGG mm -- dragging around "max-msp" as a bogus filename, but ignore this
/*
    // test it
    if( !fd ) {
...
*/
    ...
    // parse
    // parse
// BGG mm -- yyparse() takes a buffer pointe and buffer length as params
//    if( !(yyparse( ) == 0) ) goto cleanup;
    if( !(yyparse(buf, strlen(buf)+1 ) == 0) ) goto cleanup;
