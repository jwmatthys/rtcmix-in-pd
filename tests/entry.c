 ...
 * rtcmix_entry is free software; you can redistribute it and/or modify  *
 * it under the terms of the GNU General Public License as published by  *
 * the Free Software Foundation; either version 2 of the License, or     *
 * (at your option) any later version.                                   *

 * rtcmix_entry is distributed in the hope that it will be useful,       *
...
#define RTCMIX_ENTRY_DEFAULT_WIDTH     200
#define RTCMIX_ENTRY_DEFAULT_HEIGHT    100
#define RTCMIX_ENTRY_MIN_WIDTH         66
#define RTCMIX_ENTRY_MIN_HEIGHT        34
...
static t_class *rtcmix_entry_class;
...
static void rtcmix_entry_getrect(t_gobj *z, t_glist *owner, int *xp1, int *yp1, int *xp2, int *yp2);
static void rtcmix_entry_displace(t_gobj *z, t_glist *glist, int dx, int dy);
static void rtcmix_entry_select(t_gobj *z, t_glist *glist, int state);
static void rtcmix_entry_activate(t_gobj *z, t_glist *glist, int state);
static void rtcmix_entry_delete(t_gobj *z, t_glist *glist);
static void rtcmix_entry_vis(t_gobj *z, t_glist *glist, int vis);
//static int rtcmix_entry_click(t_gobj *z, t_glist *glist, int xpix, int ypix, int shift, int alt, int dbl, int doit);
static void rtcmix_entry_save(t_gobj *z, t_binbuf *b);


static t_widgetbehavior   rtcmix_entry_widgetbehavior = {
 w_getrectfn:  rtcmix_entry_getrect,
 w_displacefn: rtcmix_entry_displace,
 w_selectfn:   rtcmix_entry_select,
 w_activatefn: NULL,
 w_deletefn:   rtcmix_entry_delete,
 w_visfn:      rtcmix_entry_vis,
 w_clickfn:    NULL,
};

/* widget helper functions */
static void set_tk_widget_ids(t_rtcmix_entry *x, t_canvas *canvas)
{
...
static int calculate_onset(t_rtcmix_entry *x, t_glist *glist,
...
static void draw_inlets(t_rtcmix_entry *x, t_glist *glist, int firsttime,
...
static void erase_inlets(t_rtcmix_entry *x)
{
...
static void draw_scrollbar(t_rtcmix_entry *x)
{
...
static void erase_scrollbar(t_rtcmix_entry *x)
{
  sys_vgui("pack forget %s \n", x->scrollbar_id);
  x->x_have_scrollbar = 0;
}

static void bind_button_events(t_rtcmix_entry *x)
{
  /* mouse buttons */
...
static void rtcmix_entry_drawme(t_rtcmix_entry *x, t_glist *glist, int firsttime)
{
  DEBUG(post("rtcmix_entry_drawme: firsttime %d canvas %lx glist %lx", firsttime, x->x_canvas, glist););
...
static void rtcmix_entry_erase(t_rtcmix_entry* x,t_glist* glist)
{
  DEBUG(post("rtcmix_entry_erase: canvas %lx glist %lx", x->x_canvas, glist););
...
static void rtcmix_entry_getrect(t_gobj *z, t_glist *owner,
                                 int *xp1, int *yp1, int *xp2, int *yp2)
{
  //    DEBUG(post("rtcmix_entry_getrect");); /* this one is very chatty :D */
  t_rtcmix_entry *x = (t_rtcmix_entry*)z;
...
{
  t_rtcmix_entry *x = (t_rtcmix_entry *)z;
  DEBUG(post("rtcmix_entry_displace: canvas %lx glist %lx", x->x_canvas, glist););
  x->x_obj.te_xpix += dx;
  x->x_obj.te_ypix += dy;
...
{
  t_rtcmix_entry *x = (t_rtcmix_entry *)z;
  DEBUG(post("rtcmix_entry_select: canvas %lx glist %lx state %d", x->x_canvas, glist, state););
  ...
  rtcmix_entry_activate(z, glist, state);
}

static void rtcmix_entry_activate(t_gobj *z, t_glist *glist, int state)
{
  DEBUG(post("rtcmix_entry_activate %d", state););
  t_rtcmix_entry *x = (t_rtcmix_entry *)z;
  int x1, y1, x2, y2;

  if(state)
    {
      rtcmix_entry_getrect(z, glist, &x1, &y1, &x2, &y2);
...
static void rtcmix_entry_delete(t_gobj *z, t_glist *glist)
{
  DEBUG(post("rtcmix_entry_delete: glist %lx", glist););
  t_text *x = (t_text *)z;
...
{
  t_rtcmix_entry *x = (t_rtcmix_entry*)z;
  DEBUG(post("rtcmix_entry_vis: vis %d canvas %lx glist %lx", vis, x->x_canvas, glist););
  if (vis) {
    rtcmix_entry_drawme(x, glist, 1);
  }
  else {
    rtcmix_entry_erase(x, glist);
  }
}

/*  the clickfn is only called in run mode
    static int rtcmix_entry_click(t_gobj *z, t_glist *glist, int xpix, int ypix,
    int shift, int alt, int dbl, int doit)
    {
    t_rtcmix_entry *x = (t_rtcmix_entry *)z;
    DEBUG(post("rtcmix_entry_click x:%d y:%d edit: %d", xpix, ypix, x->x_canvas->gl_edit););
    return 0;
    }
*/

static void rtcmix_entry_append(t_rtcmix_entry* x,  t_symbol *s, int argc, t_atom *argv)
{
  DEBUG(post("rtcmix_entry_append"););
  int i;
...
static void rtcmix_entry_key(t_rtcmix_entry* x,  t_symbol *s, int argc, t_atom *argv)
{
  DEBUG(post("rtcmix_entry_key"););
  t_symbol *tmp_symbol = s; /* <-- this gets rid of the unused variable warning */
  t_int tmp_int;
...
static void rtcmix_entry_clear(t_rtcmix_entry* x)
{
  sys_vgui("%s delete 0.0 end \n", x->text_id);
}

/* Function to reset the contents of the rtcmix_entry box */
static void rtcmix_entry_set(t_rtcmix_entry* x,  t_symbol *s, int argc, t_atom *argv)
{
  DEBUG(post("rtcmix_entry_set"););

  rtcmix_entry_clear(x);
  rtcmix_entry_append(x, s, argc, argv);
}

static void rtcmix_entry_output(t_rtcmix_entry* x, t_symbol *s, int argc, t_atom *argv)
{
  outlet_list(x->x_data_outlet, s, argc, argv );
}

/* Pass the contents of the text widget onto the rtcmix_entry_output fuction above */
static void rtcmix_entry_bang_output(t_rtcmix_entry* x)
{
...
static void rtcmix_entry_keyup(t_rtcmix_entry *x, t_float f)
{
  /*     DEBUG(post("rtcmix_entry_keyup");); */
  int keycode = (int) f;
  char buf[10];
...
static void rtcmix_entry_option_float(t_rtcmix_entry* x, t_symbol *option, t_float value)
{
  sys_vgui("%s configure -%s %f \n",
           x->text_id, option->s_name, value);
}

static void rtcmix_entry_option_symbol(t_rtcmix_entry* x, t_symbol *option, t_symbol *value)
{
  sys_vgui("%s configure -%s {%s} \n",
           x->text_id, option->s_name, value->s_name);
}

static void rtcmix_entry_option(t_rtcmix_entry *x, t_symbol *s, int argc, t_atom *argv)
{
  t_symbol *tmp_symbol = s; /* <-- this gets rid of the unused variable warning */
  ...
  if(tmp_symbol == &s_)
    {
      rtcmix_entry_option_float(x,atom_getsymbolarg(0, argc, argv),
                                atom_getfloatarg(1, argc, argv));
    }
  else
    {
      rtcmix_entry_option_symbol(x,atom_getsymbolarg(0, argc, argv),tmp_symbol);
    }
}

static void rtcmix_entry_scrollbar(t_rtcmix_entry *x, t_float f)
{
  if(f > 0)
    draw_scrollbar(x);
  else
...
void rtcmix_entry_bgcolour(t_rtcmix_entry* x, t_symbol* bgcol)
{
  x->x_bgcolour = bgcol;
...
void rtcmix_entry_fgcolour(t_rtcmix_entry* x, t_symbol* fgcol)
{
  x->x_fgcolour = fgcol;
...
static void rtcmix_entry_fontsize(t_rtcmix_entry *x, t_float font_size)
{
  DEBUG(post("rtcmix_entry_fontsize"););
  if(font_size > 8)
    {
    ...
    pd_error(x,"rtcmix_entry: invalid font size: %f",font_size);
}

static void rtcmix_entry_size(t_rtcmix_entry *x, t_float width, t_float height)
{
  DEBUG(post("rtcmix_entry_size"););
  x->x_height = height;
  x->x_width = width;
...
/* callback functions */
static void rtcmix_entry_click_callback(t_rtcmix_entry *x, t_floatarg f)
{
  if( (x->x_glist->gl_edit) && (x->x_glist == x->x_canvas) )
    {
      rtcmix_entry_select((t_gobj *)x, x->x_glist, f);
    }
}

static void rtcmix_entry_resize_click_callback(t_rtcmix_entry *x, t_floatarg f)
{
...
static void rtcmix_entry_resize_motion_callback(t_rtcmix_entry *x, t_floatarg f1, t_floatarg f2)
{
  DEBUG(post("rtcmix_entry_resize_motion_callback"););
  if (x->x_resizing)
    {
...
static void rtcmix_entry_free(t_rtcmix_entry *x)
{
  pd_unbind(&x->x_obj.ob_pd, x->x_receive_name);
}

static void *rtcmix_entry_new(t_symbol *s, int argc, t_atom *argv)
{
  DEBUG(post("rtcmix_entry_new"););
  t_rtcmix_entry *x = (t_rtcmix_entry *)pd_new(rtcmix_entry_class);
  char buf[MAXPDSTRING];
  ...
  if (argc < 4)
    {
      //post("rtcmix_entry: You must enter at least 4 arguments. Default values used.");
      x->x_width = RTCMIX_ENTRY_DEFAULT_WIDTH;
      x->x_height = RTCMIX_ENTRY_DEFAULT_HEIGHT;
  ...
  sprintf(buf,"rtcmix_entry%lx",(long unsigned int)x);
...
void rtcmix_entry_setup(void) {
  rtcmix_entry_class = class_new(gensym("rtcmix_entry"), (t_newmethod)rtcmix_entry_new,
                                 (t_method)rtcmix_entry_free, sizeof(t_rtcmix_entry),0,A_GIMME,0);

  class_addbang(rtcmix_entry_class, (t_method)rtcmix_entry_bang_output);

  class_addmethod(rtcmix_entry_class, (t_method)rtcmix_entry_keyup,
                  gensym("keyup"),
                  A_DEFFLOAT,
                  0);

  class_addmethod(rtcmix_entry_class, (t_method)rtcmix_entry_scrollbar,
                  gensym("scrollbar"),
                  A_DEFFLOAT,
                  0);

  class_addmethod(rtcmix_entry_class, (t_method)rtcmix_entry_option,
                  gensym("option"),
                  A_GIMME,
                  0);

  class_addmethod(rtcmix_entry_class, (t_method)rtcmix_entry_size,
                  gensym("size"),
                  A_DEFFLOAT,
                  A_DEFFLOAT,
                  0);

  class_addmethod(rtcmix_entry_class, (t_method)rtcmix_entry_fontsize,
                  gensym("fontsize"),
                  A_DEFFLOAT,
                  0);

  class_addmethod(rtcmix_entry_class, (t_method)rtcmix_entry_output,
                  gensym("output"),
                  A_GIMME,
                  0);

  class_addmethod(rtcmix_entry_class, (t_method)rtcmix_entry_set,
                  gensym("set"),
                  A_GIMME,
                  0);

  class_addmethod(rtcmix_entry_class, (t_method)rtcmix_entry_append,
                  gensym("append"),
                  A_GIMME,
                  0);

  class_addmethod(rtcmix_entry_class, (t_method)rtcmix_entry_key,
                  gensym("key"),
                  A_GIMME,
                  0);

  class_addmethod(rtcmix_entry_class, (t_method)rtcmix_entry_clear,
                  gensym("clear"),
                  0);

  class_addmethod(rtcmix_entry_class, (t_method)rtcmix_entry_bgcolour,
                  gensym("bgcolour"),
                  A_DEFSYMBOL,
                  0);

  class_addmethod(rtcmix_entry_class, (t_method)rtcmix_entry_fgcolour,
                  gensym("fgcolour"),
                  A_DEFSYMBOL,
                  0);

  class_addmethod(rtcmix_entry_class, (t_method)rtcmix_entry_click_callback,
                  gensym("click"), A_FLOAT, 0);
  class_addmethod(rtcmix_entry_class, (t_method)rtcmix_entry_resize_click_callback,
                  gensym("resize_click"), A_FLOAT, 0);
  class_addmethod(rtcmix_entry_class, (t_method)rtcmix_entry_resize_motion_callback,
                  gensym("resize_motion"), A_FLOAT, A_FLOAT, 0);

  class_setwidget(rtcmix_entry_class,&rtcmix_entry_widgetbehavior);
  class_setsavefn(rtcmix_entry_class,&rtcmix_entry_save);
